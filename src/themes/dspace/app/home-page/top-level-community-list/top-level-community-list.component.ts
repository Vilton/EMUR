...
@Component({
  ...
  standalone: true,
  imports: [VarDirective, NgIf, ErrorComponent, ThemedLoadingComponent, AsyncPipe, TranslateModule, CommonModule, InfiniteScrollModule],
})
...
export class TopLevelCommunityListComponent implements OnInit, OnDestroy {
  ...
  initPage() {
    ...
    this.currentPageSubscription = observableCombineLatest([pagination$, sort$]).pipe(
      ...
      this.communitiesRD$.getValue().payload?.page.forEach((x, i, a) => {
        fetch(x._links.logo.href)
          .then((response) => response.json()
            .then(y => {
              this.communitiesRD$.value.payload.page[i].logo = y._links.content.href
            })).catch(() => { }).finally(() => this.cdRef.markForCheck())
      })
